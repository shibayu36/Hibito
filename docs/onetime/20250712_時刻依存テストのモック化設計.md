...
**ファイル配置**: `ViewModels/DateProvider.swift`（新規作成）
...
struct MockDateProvider: DateProvider {
    ...
    /// 便利な初期化メソッド：時分指定で今日の特定時刻を作成
    init(hour: Int, minute: Int = 0, second: Int = 0) {
        let calendar = Calendar.current
        let today = Date()
        self.fixedDate = calendar.date(bySettingHour: hour, minute: minute, second: second, of: today) ?? today
    }
}
```
...
### Phase 1: DateProviderプロトコルの導入
**作業内容**:
1. `ViewModels/DateProvider.swift`ファイルを新規作成
2. `DateProvider`プロトコル、`SystemDateProvider`、`MockDateProvider`を実装
3. コンパイルエラーがないことを確認

**検証方法**:
- プロジェクトがビルドできること
- 新しいファイルが適切にターゲットに追加されていること
...
### ファイル配置の考慮事項
- `DateProvider.swift`は`ViewModels/`ディレクトリに配置
- プロジェクトファイル(.xcodeproj)への適切な追加が必要
- テストターゲットからもアクセス可能にする設定

## 影響範囲まとめ

### 修正が必要なファイル
1. **新規作成**: `ViewModels/DateProvider.swift`
2. **修正**: `ViewModels/TodoListViewModel.swift` (4箇所)
3. **修正**: `HibitoTests/ViewModels/TodoListViewModelTests.swift` (2つのテストケース)
