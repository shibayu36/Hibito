...
Simulatorでの自動確認は行う必要はなく、動かしたい場合はわたしに知らせて。

### Xcodeプロジェクトへのファイル追加
このプロジェクトはXcodeの「File System Synchronized Groups」を使用しているため、`Hibito/`や`HibitoTests/`配下にSwiftファイルを置くだけで自動的にXcodeが認識する。`project.pbxproj`を手動で編集したり、`mcp__xcodeproj__add_file`でファイルを追加する必要はない。

## ファイル調査時の注意
docs/onetime/配下のファイルは一度きりの設計にも使っていて古い可能性があるので、こちらから明示的に読み込むことを指定した時のみ参照してください。
...
#### テスト品質向上の原則
**⚠️ 重要**: テストの信頼性を確保するため、以下の原則を必ず守ること

**テストと実装の独立性**:
- **実装とは異なる方法で期待値を作成**：テストと実装で同じロジックを使うとバグが検出できない
- **具体例**：ISO8601パーサーをテストする場合、期待値はDateComponentsで作成する
- **検証対象の明確化**：何をテストしているかを明確にし、実装の内部詳細に依存しない

**最小限のテスト設計**:
- **重複検証の排除**：同じことを複数箇所でテストしない
- **必要十分な検証**：過剰なテストケースは避け、重要な動作に絞る
- **テストのシンプル化**：不要な変数や冗長なアサーションを削除
...
let expectedDate = ISO8601DateFormatter().date(from: "2025-01-14T15:30:00+09:00")  // 同じパーサー使用
...
let expectedDate = Calendar.current.date(from: DateComponents(
    ...
    year: 2025, month: 1, day: 14, hour: 15, minute: 30, second: 0
))!  // DateComponentsで独立して作成
```
...
#### テストの表現力
- **テストメソッド名や@Testは仕様**：テストメソッド名や@Testで何をテストしているかを明確に表現
...
#### 実装前調査の重要性
**⚠️ 重要**: 大規模な変更や新機能開発では、実装前の詳細調査が成功の鍵となる

**既存コードの詳細調査**:
- **具体的な問題箇所の特定**：単に「時刻依存処理がある」ではなく「TodoListViewModel.swift:138行目でDate()直接使用」まで具体化
- **影響範囲の正確な把握**：修正が必要なファイル vs 影響を受けないファイルを明確に分類
- **実装パターンの理解**：既存のアーキテクチャパターンを理解してから新しい設計を検討

**設計文書の段階的具体化**:
- **調査結果での設計補完**：初期設計文書を実際のコード調査結果で詳細化
- **実装可能レベルまでの具体化**：「DateProviderパターンを導入」→「TodoListViewModel.swift の4箇所の具体的修正内容」
- **段階的実装計画の策定**：Phase 1〜5のような安全な移行手順の設計

**調査プロセスの実例**:
```
1. 既存コード調査 → TodoListViewModelの138行目でDate()直接使用を発見
2. テスト影響分析 → 4つのテストケース中2つが影響、2つは影響なしを特定
3. 設計詳細化 → DateProviderプロトコルの完全なコード例を作成
4. 実装計画策定 → Phase 1: プロトコル導入 → Phase 2: ViewModel修正 → ...
```

**事前調査の効果**:
- **リスク軽減**：実装中の予期しない問題を大幅に削減
- **作業効率向上**：実装時に迷わず進められる詳細設計
- **品質向上**：既存パターンとの整合性を保った設計
...
### 批判的思考と建設的な提案
**⚠️ 重要**: 指示や提案を鵜呑みにせず、批判的に検討することで品質向上を図る

**指示・提案の批判的検討**:
- **間違いの積極的指摘**：指示をする側も人間で間違うことがあるため、疑問点は遠慮なく指摘
- **背景確認の重要性**：説明不足や不明な点があれば、実装前に必ず確認をとる
- **実装方針の検証**：「なぜその方法なのか」「他に良い方法はないか」を常に考える

**代替案の提案**:
- **複数案の検討**：指示された案以外にも代替案がないかを積極的に考える
- **メリット・デメリットの明確化**：各案の利点と欠点を整理して提示
- **建設的な提案**：批判するだけでなく、より良い解決策を提案する

**効果的なコミュニケーション例**:
```
❌ 指示を鵜呑みにする例：
「MockDateProviderの便利な初期化メソッドを追加して」
→ そのまま実装

✅ 批判的検討の例：
「MockDateProviderの便利な初期化メソッドを追加して」
→ 「便利な初期化メソッドより、デフォルト引数で現在時刻にセットする方がシンプルでは？」
→ より良い設計への修正
```

**質の高い協働のために**:
- **遠慮のない意見交換**：上下関係に関係なく、技術的な意見は率直に伝える
- **根拠に基づく議論**：感情ではなく、技術的根拠に基づいて議論する
- **学習機会として捉える**：異なる意見を学習と成長の機会と考える
...
├── DateProvider.swift      # 時刻取得の抽象化プロトコル
├── Models/
...
```
HibitoTests/
├── DateProviderTests.swift # DateProvider.swiftのテスト
├── Repositories/
